                    ...
                    with open(reference_file, 'r') as ref_file:
                        for seq_record in SeqIO.parse(ref_file, 'fasta'):
                            if seq_record.id == alignment.hit_id:
                                ...
                                upstream_length = 100
                                if hsp.frame[1] > 0:  # Positive strand
                                    upstream_start = max(0, hsp.sbjct_start - upstream_length) - 1
                                    upstream_end = hsp.sbjct_start - 2
                                else:  # Negative strand
                                    upstream_start = hsp.sbjct_end
                                    upstream_end = min(len(seq_record.seq), hsp.sbjct_end + upstream_length) - 1
